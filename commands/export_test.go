...
func TestExportNotesFile(t *testing.T) {
	notes := []store.Note{
	...
	dir := t.TempDir()
	path, err := exportNotesFile(notes, []string{"#auth"}, dir, "2026-02-27T14-30-00")
	if err != nil {
...
func TestExportNotesFileCreatesDir(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "nested", "exports")
	notes := []store.Note{
		{ID: 1, Text: "a note", Tags: []string{"#todo"}, CreatedAt: "2026-02-27 10:00:00"},
	}
	_, err := exportNotesFile(notes, []string{"#todo"}, dir, "2026-02-27T14-30-00")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
